...
function _getArticleNormalizedRanksFromMySQL(
    ...
    app.mysqlPool.query(
        `SELECT wikidata_id, normalized_rank, source.code as source_language FROM ??
            INNER JOIN ?? as target ON ??=target.id
            INNER JOIN ?? as source ON ??=source.id
            WHERE wikidata_id in (?)
            AND target.??=? ORDER BY normalized_rank DESC LIMIT 10`,
        [normalizedRankTable, langTable,
            `${normalizedRankTable}.target_id`, langTable,
            `${normalizedRankTable}.source_id`, wikidataIds,
            'code', targetLanguage],
        (error, results, _) => {
            if (error) {
