...
function getArticlesByPageviews(app, source, projectDomain) {
    ...
    let requests = [];
    for (let i = 0; i < 10; i++) {
        parameters.gpvimoffset = i * 50;
        requests.push(getArticles(app, domain, parameters));
    }
    return BBPromise.all(requests.map(p => p.catch(e => e)))
    .then(results => {
        const validResults = results.filter(result => !(result instanceof Error));
        return validResults.reduce((accumulator, result) => {
            Object.keys(result).map((id) => {
                accumulator[id] = result[id];
            });
            return accumulator;
        }, {});
    });
...
function filter(app, source, target, projectDomain, candidates) {
    ...
    const sizeOfChunk = 50;
    const numOfChunks = items.length / sizeOfChunk;
    let chunks = [];
    ...
    const queries = chunks.map((items) => {
    ...
    const requests = queries.map(query => wdqsApiGet(app, query));
    return BBPromise.all(requests)
    .then(results => {
        return results.reduce((accumulator, result) => {
            result.body.results.bindings.map((item) => {
