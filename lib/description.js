...
function buildResponse(app, req, taskName, cond) {
    return P.join(
        ...
        .then((pages) => P.join(
            ...
            (entityData, wikidataPageQueryResponse) => {
                const entities = entityData.body.entities;
                const wikidataPageQuery = wikidataPageQueryResponse.body &&
                    wikidataPageQueryResponse.body.query;
                const wikidataPages = (wikidataPageQuery && wikidataPageQuery.pages) || [];
                pages.forEach((page) => {
